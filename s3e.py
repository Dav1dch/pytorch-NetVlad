...
root_dir = '/home/david/Data/pose_graph/color_img'
...
def get_whole_training_set(onlyDB=False):
    structFile = join(struct_dir, 's3e_train.mat')
...
def get_whole_val_set():
    structFile = join(struct_dir, 's3e_val.mat')
    return WholeDatasetFromStruct(structFile,
                                  input_transform=input_transform())
...
def get_whole_test_set():
    structFile = join(struct_dir, 's3e_test.mat')
    return WholeDatasetFromStruct(structFile,
                                  input_transform=input_transform())
...
dbStruct = namedtuple('dbStruct', ['whichSet', 'dataset',
                                   'dbImage', 'poseDb', 'qImage', 'poseQ', 'numDb', 'numQ',
...
def parse_dbStruct(path):
    ...
    whichSet = struct[0][0][0]

    dbImage = [f.strip() for f in struct[1][0]]

    poseDb = struct[2][0]

    qImage = [f.strip() for f in struct[3][0]]
    poseQ = struct[4][0]

    numDb = struct[5][0][0][0]
    numQ = struct[6][0][0][0]
    ...
    return dbStruct(whichSet, dataset, dbImage, poseDb, qImage,
                    poseQ, numDb, numQ, posDistThr,
...
def myDist(x, y):
    x_pose = x[:3]
    y_pose = y[:3]
    return np.linalg.norm(x_pose-y_pose)
...
class WholeDatasetFromStruct(data.Dataset):
    ...
    def getPositives(self):
        ...
        if self.positives is None:
            knn = NearestNeighbors(n_jobs=-1, metric=myDist)
            knn.fit(self.dbStruct.poseDb)

            self.distances, self.positives = knn.radius_neighbors(self.dbStruct.poseQ,
...
class QueryDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, nNegSample=200, nNeg=10, margin=0.1, input_transform=None):
        super().__init__()
        ...
        self.nontrivial_positives = list(knn.radius_neighbors(self.dbStruct.poseQ,
        ...
        potential_positives = knn.radius_neighbors(self.dbStruct.poseQ,
                                                   radius=self.dbStruct.posDistThr,
                                                   return_distance=False)
    ...
    def __getitem__(self, index):
        ...
        for negIndex in negIndices:
            negative = Image.open(
                join(root_dir, self.dbStruct.dbImage[negIndex]))
            if self.input_transform:
